...
class PenjualanController extends Controller
{
    ...
    public function store(Request $request, $isChecked = 'false')
    {
        ...
        foreach ($detail as $item) {
            ...
            $item->update();
            $hpp = HppProduk::where('id_produk', $item->id_produk)
                                ->where('id_hpp', $item->id_hpp) // Sesuai ID harga bahan
                                ->first();
                if ($hpp) {
                    // Tambahkan stok berdasarkan jumlah yang dibeli
                    $hpp->stok -= $item->jumlah;
                    $hpp->update();
                }
            
                LaporanPenjualan::create([
                    'nama_produk' => $item->produk->nama_produk, // Pastikan relasi produk tersedia
                    'hpp' => $hpp ? $hpp->hpp : 0, // Harga HPP
    ...
    public function notaBesar($isChecked = "false")
    {
        ...
        $pdf = PDF::loadView('penjualan.nota_besar', compact('setting', 'penjualan', 'detail', 'isChecked'));
